...
function validationErr (err) {
    ...
    for (let k in err.errors) {
        ...
        if (!Array.isArray(msg.message) && ~msg.message.indexOf('unique')) msg.message = 'уже занято';
...
module.exports = async (ctx, next) => {
    ...
    try {
        ...
        if (Array.isArray(err.message)) message = 'httpErrors.400'; // TODO: MONGODB NATIVE VALIDATOR
        ...
        if (preferredType === 'json') {
            ...
            return ctx.render('error', {
                status: ctx.status,
                message: typeof message === 'string' ? message : 'httpErrors.' + ctx.status
            });
        }
